  ...
  persist(
    (set, get) => ({
      ...
      hydrateFromLocal: ({ profiles, activeProfileId }) => {
        ...
        const nextId = resolveLocalTestActiveId(kept, preferred);
        const inboxState = stashAndLoadInbox(get(), nextId);
        set({
          profiles: kept,
          activeProfileId: nextId,
          ...
          try {
            ...
            void import("../lib/cloud").then(async (m) => {
              ...
              if (isCloudProfileId(String(activeProfileId))) {
                const key = ["xp", meta?.chapterId || "na", amount, Math.floor(Date.now() / 60_000)].join(":");
      ...
      merge: (persisted, current) => {
        try {
          ...
          return {
            ...
            profiles,
            activeProfileId,
            ligue: settleLeagueFreeze({ ...(p.ligue ?? current.ligue) }),
